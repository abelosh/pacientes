<?php
    class Categorias extends Controllers{
            public function __construct()
            {
                ...
                getPermisos(MCATEGORIAS); //id de la tabla modulos
            }
            ...
            public function Categorias()
            {
                ...
                $data['page_title'] = "CATEGORIAS <small>Atención Al Usuario</small>";
		...
		public function getSelectCategorias(){
			...
			if(count($arrData) > 0 ){
				for ($i=0; $i < count($arrData); $i++) { 
					if($arrData[$i]['status'] == 1 ){
					$htmlOptions .= '<option value="'.$arrData[$i]['idcategoria'].'">'.$arrData[$i]['nombrecat'].'</option>';
					}
				}
			}
			echo $htmlOptions;
